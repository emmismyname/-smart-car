...
void adc_aquire(int count)
{
	...
	for (i = 0; i < count; i++)
	{
		adc[1].data_0 = adc_once(ADC_P17, ADC_12BIT);
		adc[2].data_0 = adc_once(ADC_P05, ADC_12BIT);
		adc[3].data_0 = adc_once(ADC_P00, ADC_12BIT);
		adc[4].data_0 = adc_once(ADC_P11, ADC_12BIT);
		adc[5].data_0 = adc_once(ADC_P10, ADC_12BIT);

		adc[6].data_0 = adc_once(ADC_P16, ADC_12BIT);
		adc[7].data_0 = adc_once(ADC_P13, ADC_12BIT);
		adc[8].data_0 = adc_once(ADC_P06, ADC_12BIT);
		adc[9].data_0 = adc_once(ADC_P14, ADC_12BIT);
		adc[10].data_0 = adc_once(ADC_P01, ADC_12BIT);
