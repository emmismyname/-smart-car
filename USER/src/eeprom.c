...
void read_all_par() // 开机读取数据
{
	...
	sscanf(read_pid1_strline, "%f,%f,%f,%f,%f,%f,%d", &Speed_PID_L.KP, &Speed_PID_L.KI, &Speed_PID_L.KD, &Speed_PID_R.KP, &Speed_PID_R.KI, &Speed_PID_R.KD, &jiangxing_No_1.uart_mode);

	iap_read_bytes(0x100, read_pid2_strline, 256);
	printf("[read2%s])", read_pid2_strline); // 上位机串口检查写入内容
	sscanf(read_pid2_strline, "%f,%f,%f,%d", &Forward_PID.KP, &Forward_PID.KI, &Forward_PID.KD, &uart_mode);
...
//	uint8 eeprom_pid4_strline[256]; // 用静态储存储存pid参数，将浮点转换成字符串传到E^2prom
	...
	sprintf(eeprom_pid1_strline, "%f,%f,%f,%f,%f,%f,%d", Speed_PID_L.KP, Speed_PID_L.KI, Speed_PID_L.KD, Speed_PID_R.KP, Speed_PID_R.KI, Speed_PID_R.KD, jiangxing_No_1.uart_mode);
	printf("(write1%s)", eeprom_pid1_strline); // 上位机串口检查写入内容
	iap_write_bytes(0x00, eeprom_pid1_strline, 256);

	sprintf(eeprom_pid2_strline, "%f,%f,%f,%d", Forward_PID.KP, Forward_PID.KI, Forward_PID.KD, uart_mode);
