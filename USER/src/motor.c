...
void get_error(PID_P *pid,float senddata)
{
  pid->err=senddata;
}
...
void enablemotor()
{
	jiangxing_No_1.motor_flag=1;//电机软件开关
}

void disenablemotor()//电机软件失能
{
	jiangxing_No_1.motor_flag=0;//电机软件开关
}
...
{
	if (jiangxing_No_1.motor_flag == 0)
	{
		set_duty_L = 0; // 左右电机不输出
		set_duty_R = 0; // 左右电机不输出
	}
	else if (jiangxing_No_1.motor_flag == 1)
	{
		Speed_PID_L.err = jiangxing_No_1.left_target_speed- Current_Speed_L;
		Speed_PID_R.err = jiangxing_No_1.right_target_speed- Current_Speed_R;
