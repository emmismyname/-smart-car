...
void mode_choose(int move_mode) // 运动模式切换
{
	...
	else if (trace_flag == 1)
	{
		...
		{
			motor_flag = 1; // 电机输出
			Target_Speed_L = Target_speed - PID_Loc_Control(&Forward_PID, Forward_PID.err) - z_gyro * gyro_d;
			Target_Speed_R = Target_speed + PID_Loc_Control(&Forward_PID, Forward_PID.err) + z_gyro * gyro_d;
		}
		break;
		case 4: // 弯道输出
		{
			motor_flag = 1; // 电机输出
			Target_Speed_L = Target_speed - PID_Loc_Control(&Forward_PID, Forward_PID.err) - PID_Loc_Control(&Swerve_PID, err) - z_gyro * gyro_d;
			Target_Speed_R = Target_speed + PID_Loc_Control(&Forward_PID, Forward_PID.err) + PID_Loc_Control(&Swerve_PID, err) + z_gyro * gyro_d;
		}
		break;
		case 8: // 角度环调参
		{
